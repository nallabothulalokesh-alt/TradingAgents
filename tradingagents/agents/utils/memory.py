...
class TradingMemoryLog:
    ...
    def store_decision(
        ...
        # Enforce rotation before appending new entry
    ...
    def _enforce_rotation(self) -> None:
        """Enforce memory log rotation by pruning oldest resolved entries.
        
        Called before appending new entries to keep the log size bounded.
        """
        if not self._log_path or not self._log_path.exists():
            return
        ...
        text = self._log_path.read_text(encoding="utf-8")
        blocks = text.split(self._SEPARATOR)
        rotated_blocks = self._apply_rotation(blocks)
        
        # Only rewrite if rotation actually removed entries
        if len(rotated_blocks) < len(blocks):
            new_text = self._SEPARATOR.join(rotated_blocks)
            tmp_path = self._log_path.with_suffix(".tmp")
            tmp_path.write_text(new_text, encoding="utf-8")
            tmp_path.replace(self._log_path)
