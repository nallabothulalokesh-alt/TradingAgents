...
def render_compare():
    ...
    with col_a:
        sel_a = st.selectbox("Analysis A", labels, index=0, key="cmp_a")
    with col_b:
        sel_b = st.selectbox("Analysis B", labels,
    ...
    st.markdown(
        f'<div style="background:#1e293b;border-radius:10px;padding:12px 18px;'
        f'text-align:center;margin-bottom:16px;color:{verdict_color};font-size:15px">'
        f'⚖️ {verdict}</div>',
